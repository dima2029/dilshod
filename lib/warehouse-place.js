...
// Телефонные клавиатуры (особенно iPhone с «умной пунктуацией») нередко автоматически
// заменяют обычный дефис на длинное/короткое тире или минус — из-за этого «4-Н» превращается
// в «4–Н» и наш разбор перестаёт узнавать заголовок/артикул. Приводим все виды тире к обычному
// дефису перед любым разбором — единственная точка нормализации на весь модуль.
function normalizeDashes(s) {
  // ‐ hyphen, ‑ non-breaking hyphen, ‒ figure dash, – en dash, — em dash, − minus sign,
  // / слэш — тоже нередко используют вместо дефиса между цветами (205700-DKNV/BBK).
...
// Буква ловится и латиницей (A-Za-z) тоже — на латинской раскладке «В» легко набрать
// как латинскую «B» (визуально неотличимы), см. normalizeConfusableLetter ниже.
const HEADER_WITH_PREFIX_RE = /^ряд[аи]?\.?\s+(\d{1,2})\s*[-,:]?\s*([А-ЯЁа-яёA-Za-z])(?=\s|$)/i;
const HEADER_BARE_RE = /^(\d{1,2})\s*[-,:]?\s*([А-ЯЁа-яёA-Za-z])\s*$/i;

function matchHeader(line) {
  return line.match(HEADER_WITH_PREFIX_RE) || line.match(HEADER_BARE_RE);
}

// Некоторые кириллические буквы визуально неотличимы от латинских на любой раскладке
// (А/A, В/B, Е/E, К/K, М/M, Н/H, О/O) — если сотрудник печатал на латинице, буква
// в заголовке может оказаться латинской. Приводим такие буквы к кириллическим
// ТОЛЬКО для уже захваченной буквы заголовка — артикулы (которые всегда латиница) не трогаем.
const LATIN_TO_CYRILLIC_LETTER = { A: 'А', B: 'В', E: 'Е', K: 'К', M: 'М', H: 'Н', O: 'О' };
function normalizeConfusableLetter(letter) {
  const up = String(letter || '').toUpperCase();
  return LATIN_TO_CYRILLIC_LETTER[up] || up;
}
...
function looksLikeBulkPlace(text) {
  const firstLine = normalizeDashes(text).split(/\r?\n/).map(l => l.trim()).find(Boolean) || '';
...
function parseBulkPlaceBlocks(text) {
  const lines = normalizeDashes(text).split(/\r?\n/).map(l => l.trim()).filter(Boolean);
  const blocks = [];
  let current = null;
  let ignored = 0;
  for (const line of lines) {
    ...
    if (m) {
      current = { row: m[1], letter: normalizeConfusableLetter(m[2]), articles: [] };
      blocks.push(current);
      continue;
    }
    if (!current) continue;
